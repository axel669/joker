import { transform } from "./schema.js"
import { builtin, errors } from "./types.js"

// const fname = (func) => `validate_${func.replace(/\.|\-/g, "_")}`
...
const validator = (schema, closureValue, target) => {
    ...
    const { lines } = [
        ...Object.keys(closure).map(
            ...
            (name) => `const ${fname(name)} = ${closureValue(closure, name)}`
        ),
        `${target} (item) => {`,
        `const errors = []`,
        ...body,
